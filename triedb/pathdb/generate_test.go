...
func TestGenerateFromEmptySnap(t *testing.T) {
	...
	for i := 0; i < 400; i++ {
		...
		helper.addTrieAccount(fmt.Sprintf("acc-%d", i), &types.StateAccount{Flags: types.YieldDisabled, Fixed: uint256.NewInt(1), Shares: new(uint256.Int), Debt: new(uint256.Int), Root: stRoot, CodeHash: types.EmptyCodeHash.Bytes()})
	}
